...
import { BsFillTrashFill, BsFillPencilFill, BsArrowRightSquareFill } from "react-icons/bs";
    ...
    function convertTime(rawTime) {
        ...
        <thead>
          <tr>
            ...
            <th scope="col" className="to-job-col">To Job</th>
          </tr>
        </thead>
        <tbody>
          {jobs.map((job, idx) => (
            <tr key={idx} >
              ...
              <td>{job.payment_status}</td>
                {user.role === "MANAGER" && (
                  ...
                  <BsFillPencilFill className="edit-btn" onClick={() => editJob(idx)} />
                <BsFillTrashFill className="delete-btn" onClick={() => deleteJob(idx)}/>
                  </span>
                  </td>
                  )}
                  <td>
                    <span className="to-job">
                      <BsArrowRightSquareFill className="nav-btn" onClick={() => navigate(`/jobs/${job.id}`)}/>
                    </span>
                  </td>
